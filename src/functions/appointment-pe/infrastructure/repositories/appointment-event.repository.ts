...
export class AppointmentEventRepository {
  ...
  async insert(event: AppointmentEvent): Promise<void> {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS appointment_events_pe (
    ...
    await pool.execute(
      `INSERT INTO appointment_events_pe (id, scheduleId, insuredId, countryISO, status, createdAt, createdAtIso)
       VALUES (?, ?, ?, ?, ?, ?, ?)
